...
fn render_main(f: &mut Frame, app: &mut App, area: Rect) {
	...
	let cols = if wide {
		...
		Layout::default()
			.direction(Direction::Horizontal)
			.constraints([Constraint::Length(list_w as u16), Constraint::Min(20)])
			.split(area)
	};
...
fn render_action_buttons(
	...
	let draw_btn =
		|f_: &mut Frame, r: Rect, label: &str, key: &str, col: Color, active: bool| {
...
fn render_footer(f: &mut Frame, app: &mut App, area: Rect) {
	...
	if !app.msg.is_empty() {
		...
		if display.len() < area.width as usize {
			...
			f.render_widget(
				Paragraph::new(display).style(Style::default()
...
fn render_help_overlay(f: &mut Frame, area: Rect, scroll: usize) {
	...
	for &(key, desc) in HELP_LINES.iter().skip(scroll) {
		if desc.is_empty() && !key.is_empty() {
			lines.push(Line::from(Span::styled(format!("  {}", key), hdr_style)));
		} else {
			lines.push(Line::from(vec![
				Span::styled(
					format!("  {:<20}", key),
...
fn render_confirm_overlay(f: &mut Frame, area: Rect, action: &ConfirmAction, names: &[String]) {
	...
	match action {
		ConfirmAction::Update => {
			...
			lines.push(Line::from(
				"  This will sync databases and upgrade all packages.",
			));
		}
		_ => {
	...
	lines.push(Line::from(vec![
		Span::styled(
			"  [y] Confirm    ",
			Style::default().fg(act_col).add_modifier(Modifier::BOLD),
		),
...
fn render_theme_builder_overlay(f: &mut Frame, area: Rect, app: &App) {
	...
	let fields = &[
		(
			"Theme Preset",
			if app.theme_builder_selected_theme_idx < crate::theme::THEMES.len() {
				crate::theme::THEMES[app.theme_builder_selected_theme_idx]
					.name
					.to_string()
			} else {
				"custom".to_string()
		...
		(
			"Highlight FG",
			crate::theme::color_name(app.theme.highlight_fg),
		),
		(
			"Highlight BG",
			crate::theme::color_name(app.theme.highlight_bg),
		),
	...
	let fields_list = List::new(list_items).block(Block::default()
	...
	let mock_list_block = Block::default()
		...
		.title(" Mock List ")
		.border_style(Style::default().fg(app.theme.border));
	let mock_list_widget =
		Paragraph::new(mock_list_item)
